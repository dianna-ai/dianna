...
def upload_image(contents, filename):
    ...
    if contents is not None:
        try:
            ...
            print(e)
            return blank_fig(text='There was an error processing this file.')
    else:
        return blank_fig()
        ...
        try:
            if 'txt' in filename:
                ...
                if labelnames is None or labelnames == ['']:
                    return html.Div([
                        'Label file is empty, please upload a valid file'
                    ]), labelnames
...
def global_store_i(method_sel,
    ...
    # expensive query
    if method_sel == 'RISE':
        relevances = dianna.explain_image(model_path,
                                          image_test,
                                          method=method_sel,
                                          labels=labels,
                                          n_masks=n_masks,
...
@app.callback(dash.dependencies.Output('output-state-img', 'children'),
...
def update_multi_options_i(fn_m,
                           fn_i,
                           sel_methods,
                           new_model,
                           new_image,
    ...
    if (fn_m and fn_i) is not None and (sel_methods != []):
        ...
        try:
            predictions = (prepare(onnx_model).run(
                X_test[None, ...])[f'{output_node}'])
            ...
            for m in sel_methods:
                for i in range(n_rows):
                    if m == 'RISE':
                        ...
                        fig.add_trace(
                            go.Heatmap(z=utilities.fill_segmentation(
                                shap_values[i][0], segments_slic),
                                       colorscale='Bluered',
                                       showscale=False,
                                       opacity=0.7), i + 1, 2)
                    elif m == 'LIME':
...
def global_store_t(method_sel,
    ...
    # expensive query
    if method_sel == 'RISE':
        relevances = dianna.explain_text(model_runner,
                                         input_text,
                                         tokenizer,
                                         method=method_sel,
                                         labels=[pred_idx],
...
# signaling
@app.callback(dash.dependencies.Output('signal_text', 'data'),
...
@app.callback(dash.dependencies.Output('output-state-text', 'children'),
...
def update_multi_options_t(fn_m,
                           input_text,
                           sel_methods,
                           new_model,
                           new_text,
    ...
    if (fn_m and input_text) is not None and (sel_methods != []):
        ...
        try:
            ...
            for m in sel_methods:
                if m == 'LIME':
                    relevances_lime = global_store_t(m,
                                                     model_runner,
                                                     input_text,
                                                     class_name,
                                                     random_state=random_state)
                    output = _create_html(input_tokens,
                                          relevances_lime[0],
                                          max_opacity=0.8)
                    fig_l = html.Div(tuple(list(output)))

                elif m == 'RISE':
                    relevances_rise = global_store_t(m,
                                                     model_runner,
                                                     input_text,
                                                     class_name,
                                                     n_masks=n_masks,
