...
class LIMEImage:
    ...
    def explain(self,
        ...
        if return_masks:
            get_image_and_mask_kwargs = utils.get_kwargs_applicable_to_function(
                explanation.get_image_and_mask, kwargs)
            ...
            maps = [self.get_explanation_values(
                label, explanation) for label in labels]
        return maps
    ...
    def _prepare_image_data(self, input_data):
        ...
        if len(input_data['channels']) == 1:
            greyscale = True
            input_data = input_data.sel(channels=0).expand_dims(
                {'channels': 3}, axis=input_data.dims.index('channels'))
    ...
    def get_explanation_values(label: int, explanation: ImageExplanation) -> NDArray:
        ...
        """
        salience_map = np.zeros(
            explanation.segments.shape, explanation.segments.dtype)
