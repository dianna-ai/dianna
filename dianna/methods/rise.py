...
class RISEText:
    ...
    def explain(self, model_or_function, input_text, tokenizer, labels=(0,), batch_size=100):
        ...
        Args:
            ...
            Tokenizer class with tokenize and convert_tokens_to_string methods, and mask_token attribute
        ...
        self.mask_string = tokenizer.mask_token
        input_tokens = np.asarray(tokenizer.tokenize(input_text))
        text_length = len(input_tokens)
        active_p_keep = self._determine_p_keep(input_tokens, runner) if self.p_keep is None else self.p_keep
    ...
    def _calculate_mean_class_std(self, p_keep, runner, input_data, n_masks):
        batch_size = 50
        masks = self._generate_masks(input_data.shape, p_keep, n_masks)
