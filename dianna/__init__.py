...
def _get_explainer(method, kwargs, modality):
    try:
        ...
        raise XAIMethodException(f"Method {method} does not exist") from err
    try:
        method_class = getattr(method_submodule, f"{method}{modality}")
    except AttributeError as err:
        raise XAIMethodException(f"Data modality {modality} is not available for method {method}") from err
